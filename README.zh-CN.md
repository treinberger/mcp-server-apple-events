...
### v1.3.0 (增强型提醒事项)
- **优先级支持**：通过原生 EventKit API 设置优先级级别（高/中/低/无）
- **重复提醒**：支持每日、每周、每月、每年的重复，具有灵活的规则
- **位置触发器**：在到达或离开时触发的地理围栏提醒
- **标签/标签**：在备注中使用 `[#tag]` 格式存储的跨列表分类
- **子任务/清单**：在备注中存储带有进度跟踪的清单项目
- **新工具**：用于管理清单项目的 `reminders_subtasks`
- **增强型过滤**：按优先级、重复、基于位置和标签进行过滤
- **视觉指示器**：🔄 (重复), 📍 (位置), 🏷️ (标签), 📋 (子任务)
